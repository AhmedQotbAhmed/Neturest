...
if( $_SERVER['REQUEST_METHOD'] === 'POST' And isset($_POST['contact_submit'])) {
	if(!empty($_POST['name']) AND (strlen($_POST['name']) > 5) AND (filter_var($_POST['name'], FILTER_SANITIZE_STRING)))
		$sender_name  = stripslashes(strip_tags(trim($_POST['name'])));

	if(!empty($_POST['phone']) AND (strlen($_POST['phone']) > 5) AND (is_numeric($_POST['phone'])))
		$sender_phone  = stripslashes(strip_tags(trim($_POST['phone'])));

	if(!empty($_POST['message']) AND (strlen($_POST['name']) > 20) AND (filter_var($_POST['name'], FILTER_SANITIZE_STRING)))
		$message      = stripslashes(strip_tags(trim($_POST['message'])));

	if(!empty($_POST['email']) AND (strlen($_POST['name']) > 10) AND (filter_var($_POST['name'], FILTER_VALIDATE_EMAIL)))
		$sender_email = stripslashes(strip_tags(trim($_POST['email'])));

	if(!empty($_POST['subject']) AND (strlen($_POST['name']) > 20) AND (filter_var($_POST['name'], FILTER_SANITIZE_STRING)))
	...
	if(empty($errors)) {
		...
		$message = $message_meta . "<br>" . 'Message:' . "<br>" . $message .'<br>'. $sender_phone;
